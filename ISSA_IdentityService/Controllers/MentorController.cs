...
{
    [ApiController]
    public class MentorController(IMentorService service) : ApiControllerBase
    {

        [HttpGet]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Get([FromQuery] MentorQuery query)
        {
            try
            {
                var result = await service.GetPaginatedAsync(query);
                return Ok(new BaseResponseModel<PaginatedList<Mentor>?>(StatusCodes.Status200OK, data: result));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetById(string id)
        {
            try
            {
                ...
                if (result == null)
                {
                    return NotFound(new BaseResponseModel<string>(StatusCodes.Status404NotFound, "Mentor not found"));

                }
                return Ok(new BaseResponseModel<Mentor?>(StatusCodes.Status200OK, data: result));
            }
            catch (Exception ex)
            {
                return BadRequest(new BaseResponseModel<string>(StatusCodes.Status500InternalServerError, ex.Message));
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Create([FromBody] MentorModel model)
        {
            try
            {
        ...
        [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string id, [FromBody] MentorModel model)
        {
            try
            {
