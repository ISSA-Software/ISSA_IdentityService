...
{
    [ApiController]
    public class AdminController(IAdminService service) : ApiControllerBase
    {

        [HttpGet]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Get([FromQuery] AdminQuery query)
        {
            try
            {
                var result = await service.GetPaginatedAsync(query);
                return Ok(new BaseResponseModel<PaginatedList<Admin>?>(StatusCodes.Status200OK, data: result));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetById(string id)
        {
            try
            {
                ...
                if (result == null)
                {
                    return NotFound(new BaseResponseModel<string>(StatusCodes.Status404NotFound, "Admin not found"));

                }
                return Ok(new BaseResponseModel<Admin?>(StatusCodes.Status200OK, data: result));
            }
            catch (Exception ex)
            {
                return BadRequest(new BaseResponseModel<string>(StatusCodes.Status500InternalServerError, ex.Message));
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Create([FromBody] AdminModel model)
        {
            try
            {
        ...
        [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string id, [FromBody] AdminModel model)
        {
            try
            {
